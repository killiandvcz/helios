...
export const heliosMethods = {
     ...
     */
    "starling:getToken": async (context) => {
        const {starling} = context;
        try {
            const token = await new SignJWT({
                starlingId: starling.id,
                timestamp: Date.now()
            }).setProtectedHeader({alg: "HS256"}).setIssuedAt().setIssuer("helios").setAudience("starling").setExpirationTime("1h").sign(starling.helios.keys.connection).catch(e => {
                console.log(e);
            })
            console.log(token);
            context.success({
